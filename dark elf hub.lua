...
local yellowBtn = new("TextButton",{
    Text="自动黄球宠: OFF", Size=UDim2.new(0,110,0,rowH), Position=UDim2.fromOffset(5,5),
    BackgroundColor3=Color3.fromRGB(60,60,60), TextColor3=Color3.new(1,1,1), BorderSizePixel=0, TextSize=9, Parent=speedTab
})
local yellowConn, yAcc = nil, 0
local function fire20Yellow()
    for i=1,20 do ReplSt.rEvents.orbEvent:FireServer("collectOrb","Yellow Orb","City") end
end
...
-- ③ 自动买武士 10 Hz
local buySamBtn = new("TextButton",{
    Text="自动买武士: OFF", Size=UDim2.new(0,110,0,rowH), Position=UDim2.fromOffset(5,5+(rowH+gap)*2),
    BackgroundColor3=Color3.fromRGB(60,60,60), TextColor3=Color3.new(1,1,1), BorderSizePixel=0, TextSize=9, Parent=speedTab
})
local buyConn, bAcc = nil, 0
local function setBuySam(s)
    buySamBtn.Text = s and "自动买武士: ON" or "自动买武士: OFF"
    buySamBtn.BackgroundColor3 = s and Color3.fromRGB(255,80,80) or Color3.fromRGB(60,60,60)
    if buyConn then buyConn:Disconnect(); buyConn = nil end
    if s then
        bAcc = 0
        buyConn = RunSvc.Heartbeat:Connect(function(dt)
            bAcc = bAcc + dt
            if bAcc >= 1/10 then bAcc = 0; ReplSt.cPetShopRemote:FireServer(ReplSt.cPetShopFolder["Swift Samurai"]) end
        end)
    end
end
buySamBtn.MouseButton1Click:Connect(function() setBuySam(buySamBtn.Text:find("OFF") and true or false) end)

-- ④ 自动进阶武士 4 Hz
local evolveBtn = new("TextButton",{
    Text="自动进阶武士: OFF", Size=UDim2.new(0,120,0,rowH), Position=UDim2.fromOffset(5,5+(rowH+gap)*3),
    BackgroundColor3=Color3.fromRGB(60,60,60), TextColor3=Color3.new(1,1,1), BorderSizePixel=0, TextSize=9, Parent=speedTab
})
local evolveConn, eAcc = nil, 0
local function setEvolve(s)
    ...
    if s then
        eAcc = 0
        evolveConn = RunSvc.Heartbeat:Connect(function(dt)
            eAcc = eAcc + dt
            if eAcc >= 1/4 then eAcc = 0; ReplSt.rEvents.petEvolveEvent:FireServer("evolvePet","Swift Samurai") end
        end)
    end
end
